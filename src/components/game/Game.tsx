...
import { GameEngine, initialBattle, PLAYER_MAX_HP } from "@/game/engine/GameEngine";
...
export function Game() {
  ...
  const enemy = getStage(battle.stage);
  const nextStage = () => {
    if (engine.current?.nextStage()) { audio.current?.start(); setRound((value) => value + 1); }
  };
  const newRun = () => { audio.current?.start(); engine.current?.start(1); setRound((value) => value + 1); };
  ...
  const message = { idle: "전투 준비", playing: "적의 공격 타이밍을 확인하세요", victory: `${enemy.name} 처치 · 전투 종료`, gameover: "플레이어 쓰러짐 · 전투 종료" }[battle.status];
    ...
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-3" onClick={(event) => {
      ...
      <header className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <span data-testid="stage" className="text-xs tracking-[.15em] text-slate-400">STAGE {String(battle.stage).padStart(2, "0")} / {STAGE_COUNT} · {formatTime(battle.elapsedMs)}</span>
        </div>
      </header>
      ...
      <section aria-label="전투" className="arena relative min-h-[340px] overflow-hidden rounded-t-xl border border-slate-700/60">
        ...
        {battle.status === "idle" && <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#080b12]/80 px-6 text-center">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-300">기술명 입력 후 Enter로 발동합니다.<br />서로 다른 10명의 적, 점점 빨라지는 전투.<br />스테이지마다 HP는 회복됩니다. 마지막 군주까지 도전하세요.</p>
        ...
        <div className="pointer-events-none absolute left-6 top-5">
          <p className="eyebrow">{battle.stage === STAGE_COUNT ? "FINAL BATTLE" : "BATTLE ARENA"}</p><p className="mt-1 text-lg font-semibold">STAGE <span className="text-orange-300">{String(battle.stage).padStart(2, "0")}</span></p>
        </div>
        <div className="pointer-events-none absolute left-1/2 top-5 w-72 -translate-x-1/2 text-center">
          <div className="mb-2 flex items-end justify-between"><h2 data-testid="enemy-name" className="text-lg font-bold">{enemy.name}</h2><span data-testid="enemy-hp" className="font-mono text-xs text-slate-300">{battle.enemyHp} / {enemy.maxHp}</span></div>
          <progress aria-label={`${enemy.name} HP`} className="hp enemy" max={enemy.maxHp} value={battle.enemyHp} />
          <div className="mt-2 flex items-center gap-3">
            <progress aria-label="적 공격까지 남은 시간" className={`timer ${battle.attackRemaining < 1500 ? "danger" : ""}`} max={enemy.attackInterval} value={battle.attackRemaining} />
        ...
        <div className="absolute bottom-5 right-6 text-right text-xs leading-6 text-slate-400">공격 주기 <span className="text-slate-200">{(enemy.attackInterval / 1000).toFixed(1)}초</span><br />공격 피해 <span className="text-slate-200">{enemy.attackDamage}</span></div>
      </section>
      <section className="rounded-b-xl border border-t-0 border-slate-700/60 bg-[#121621] px-5 py-3" aria-label="기술 입력">
        ...
        {ended ? <ResultScreen battle={battle} onRetry={restart} onNext={nextStage} onNewRun={newRun} /> : <>
